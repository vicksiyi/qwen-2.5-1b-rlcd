...
打开 [海龟汤 Demo](http://127.0.0.1:8000/turtle.html)，可选择内置汤题，或自行录入**汤面和汤底**。本地 Qwen / RLCD 根据隐藏事实判断是、否、无关、需要澄清或无法确定，支持问答记录、提交还原和揭晓汤底。

自定义内容保存在本地服务内存，开局后录入框清空，普通问答响应不返回汤底。小模型仍可能误判；使用方式和限制见 [海龟汤说明](docs/turtle-demo.md)。

快速体验：运行 `./start.sh` → 打开 `/turtle.html` → 选择“录入汤面与汤底” → 填写故事并开局 → 输入是非问题。也可选择三道内置题，查看预设提示或提交完整还原。

模型每轮根据隐藏事实与近期问答，为五类回答评分；界面展示原始候选分数和推理耗时。内容暂不持久化，重启服务后清空，无需新增依赖或 API 密钥。
